...
class AudioProcessor:
    ...
    def trim_silence(
        ...
        # Librosa expects a 1D (mono) signal for many effects utilities.
        # If we have multi-channel audio, convert to mono for trimming.
        ...
        trimmed, _ = librosa.effects.trim(mono, top_db=abs(threshold_db))

        # If original was multi-channel, map the trimmed mono region back to all channels
        if isinstance(data, np.ndarray) and data.ndim > 1:
            # Determine number of samples trimmed
            start = 0
            # librosa.effects.trim returns (y_trimmed, index)
            # The index contains (start, end) sample indices; we asked for the trimmed signal only
            # Recompute indices by finding where trimmed appears in mono
            # Safer approach: compute non-silent frame indices directly
            non_silent = librosa.effects.split(mono, top_db=abs(threshold_db))
