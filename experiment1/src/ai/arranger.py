...
import numpy as np
import librosa
from ..audio.demucs_wrapper import DemucsSeparator
from ..audio.ai_blend import AITransitionBlender
import tempfile
...
class SongArranger:
    ...
    def __init__(self, sample_rate: int = 44100, min_demucs_length_s: float = 0.5):
        self.sample_rate = sample_rate
        self._beat_times = None  # Cached beat times
        self._tempo = None  # Cached tempo
        self._bar_times = None  # Cached bar (downbeat) times

        # Initialize the Demucs separator for stem separation
        self.demucs_separator = DemucsSeparator()

        # Minimum section length (seconds) required to run Demucs. Shorter sections will skip Demucs.
        self.min_demucs_length_s = float(min_demucs_length_s)
    ...
    def detect_beat_grid(self, audio_data: np.ndarray) -> Tuple[float, np.ndarray, np.ndarray]:
        ...
        # Ensure mono input for librosa beat detection
        if getattr(audio_data, 'ndim', 1) == 2:
            y_mono = librosa.to_mono(audio_data.T)
        else:
            y_mono = audio_data
    ...
    def analyze_sections(
        ...
        external_sections: Optional[list] = None
    ) -> List[Section]:
        """
        ...
        """
        if external_sections is not None:
            ...
            for s in external_sections:
                if isinstance(s, Section):
                    ...
                    raise ValueError("Invalid section format for external_sections")
            return sections
        else:
            duration = len(audio_data) / self.sample_rate
            # If the provided audio is very short, skip heavy segmentation and
            # return a single section covering the whole audio. This avoids
            # calling STFT/MFCC on tiny buffers which can produce warnings/errors
            # and pathological segment boundaries.
            if duration < 8.0:
                mono = audio_data
                if getattr(audio_data, 'ndim', 1) == 2:
                ...
                # Create a single phrase covering the whole section
                sec.phrases = [Phrase(start_time=0.0, end_time=duration, bar_start=0, num_bars=0, energy=energy, is_downbeat_aligned=False)]
                return [sec]
            # Set defaults
            ...
            # Guard against extremely long audio causing huge STFT/MFCC allocations by
            # computing spectral features on a downsampled / truncated representative copy.
            MAX_ANALYSIS_SECONDS = 120  # seconds - cap for heavy spectral analysis
            target_frames = 5000

            # Convert to mono for feature extraction (librosa expects 1D arrays)
            ...
            if duration > MAX_ANALYSIS_SECONDS:
                self.logger.warning(
                    "Audio too long for full spectral analysis (%.1fs). "
                    "Using downsampled/truncated representative audio for features.", duration
                )
                # Downsample to a reasonable analysis sample rate to reduce memory
                analysis_sr = min(self.sample_rate, 22050)
                try:
                    y_ds = librosa.resample(y_mono, orig_sr=self.sample_rate, target_sr=analysis_sr)
                except Exception:
                    # Fallback: use a simple decimation if resample fails
                    factor = max(1, int(self.sample_rate / analysis_sr))
                    y_ds = y_mono[::factor]
                max_samples = int(MAX_ANALYSIS_SECONDS * analysis_sr)
                y_for_features = y_ds[:max_samples]
                used_sr = analysis_sr
            else:
                y_for_features = y_mono
                used_sr = self.sample_rate

            # Use adaptive hop_length for the (possibly reduced) feature buffer
            hop_length = max(512, int(len(y_for_features) / target_frames))

            # Get multiple feature types for better segmentation using the safe buffer
            mfcc = librosa.feature.mfcc(y=y_for_features, sr=used_sr, n_mfcc=13, hop_length=hop_length)
            chroma = librosa.feature.chroma_cqt(y=y_for_features, sr=used_sr, hop_length=hop_length)
            rms = librosa.feature.rms(y=y_for_features, hop_length=hop_length)[0]
            spectral_contrast = librosa.feature.spectral_contrast(y=y_for_features, sr=used_sr, hop_length=hop_length)
            # Stack features
            ...
            for i in range(len(bound_times) - 1):
                ...
                # Detect key at start and end (first/last 2s)
                slice_len = int(self.sample_rate * 2)
                start_slice = segment_audio[:slice_len] if len(segment_audio) > slice_len else segment_audio
                end_slice = segment_audio[-slice_len:] if len(segment_audio) > slice_len else segment_audio
                start_key = analyzer.detect_key_short(start_slice)
                end_key = analyzer.detect_key_short(end_slice)
                section = Section(
    ...
    def analyze_sections_with_stems(self, audio_data: np.ndarray, stems: Dict[str, np.ndarray]) -> List[Section]:
        """
        Analyze sections using both the original audio and separated stems.

        Args:
            audio_data: Raw audio data.
            stems: Dictionary of separated stems (e.g., vocals, drums, bass).

        Returns:
            List of detected sections with enhanced analysis.
        """
        # Analyze sections using the original audio
        sections = self.analyze_sections(audio_data)

        # Enhance section analysis using stems
        for section in sections:
            stem_features = {}
            for stem_name, stem_audio in stems.items():
                # Extract features from each stem
                stem_features[stem_name] = self._analyze_section_features(stem_audio)

            # Combine stem features with section features
            section.features.update(stem_features)
    ...
    def _analyze_section_features(self, audio_data: np.ndarray) -> Dict[str, float]:
        ...
        features = {}
        # Defensive: if the section is very short, avoid heavy spectral calls
        if audio_data is None or audio_data.size == 0 or len(audio_data) < 2048:
            # compute a simple RMS energy and return minimal features
            mono = audio_data
            if getattr(audio_data, 'ndim', 1) == 2:
                mono = np.mean(audio_data, axis=1)
            energy = float(np.sqrt(np.mean(mono**2))) if mono.size else 0.0
        ...
        # Convert to mono for spectral features
        if getattr(audio_data, 'ndim', 1) == 2:
            y_mono = librosa.to_mono(audio_data.T)
        else:
            y_mono = audio_data
        ...
        # Spectral centroid (brightness) -- guard n_fft/hop lengths via librosa defaults
        centroid = librosa.feature.spectral_centroid(y=y_mono, sr=self.sample_rate)[0]
        ...
        # Harmonic content
        harmonic = librosa.effects.harmonic(y_mono)
        features["harmonicity"] = float(np.mean(np.abs(harmonic)) / (np.mean(np.abs(y_mono)) + 1e-6))
    ...
    def _analyze_section_features_with_natten(self, audio_data: np.ndarray) -> Dict[str, float]:
        """
        Analyze section features using NATTEN for contextual feature extraction.

        Args:
            audio_data: Audio data for the section.

        Returns:
            Dictionary of extracted features.
        """
        # Placeholder for NATTEN-based feature extraction
        # Replace with actual NATTEN integration
        features = {
            "natten_context": 0.0,  # Example feature
        }
        return features
    ...
    def _compute_phrase_chroma(self, audio_data: np.ndarray, phrase: Phrase) -> np.ndarray:
        ...
        if getattr(phrase_audio, 'ndim', 1) == 2:
            phrase_mono = librosa.to_mono(phrase_audio.T)
        else:
            phrase_mono = phrase_audio

        chroma = librosa.feature.chroma_cqt(y=phrase_mono, sr=self.sample_rate)
    ...
    def _compute_phrase_timbre(self, audio_data: np.ndarray, phrase: Phrase) -> np.ndarray:
        ...
        mfcc = librosa.feature.mfcc(y=phrase_mono, sr=self.sample_rate, n_mfcc=13)
    ...
    def create_simple_arrangement(
        ...
        if target_duration is None:
            ...
            self.logger.info("   Quality-based trimming (keeping all good material)...")
        else:
            trim_to_target = True
            self.logger.info("   Quality-based trimming (preserving natural flow)...")
        
        self.logger.info("   Target: ~%0.0fs from %0.0fs available", target_duration, total_duration_available)
        ...
        self.logger.info("   Quality: min=%.1f, max=%.1f, avg=%.1f, range=%.1f", min(qualities), max(qualities), avg_quality, quality_range)
        ...
        if len(sorted_qualities) >= 3:
            ...
            if sorted_qualities[0] < median_q - 15:
                ...
                self.logger.info("   Found weak outliers below %.1f", quality_threshold)
        ...
        for section, quality in scored_sections:
            if has_weak_outliers and quality < quality_threshold:
                self.logger.info("   ✗ Removing weak section %.0fs-%.0fs (quality=%.1f)", section.start_time, section.end_time, quality)
            else:
                good_sections.append((section, quality))
        
        self.logger.info("   Keeping %d of %d sections", len(good_sections), len(scored_sections))
        ...
        if trim_to_target:
            ...
            while current_duration > target_duration * overshoot_factor and len(good_sections) > min_keep:
                ...
                self.logger.info("   ✗ Trimming %.0fs-%.0fs to reduce length (quality=%.1f)", removed[0].start_time, removed[0].end_time, removed[1])
        ...
        if allow_rearrange and len(good_sections) >= 3:
            ...
            self.logger.info("   Rearranged sections for better flow")
        ...
        # Show what we kept
        self.logger.info("   Final: %d sections, %0.0fs", len(arrangement_sections), arr_time)
        kept_times = [f"{s.start_time:.0f}-{s.end_time:.0f}s" for s, _, _ in arrangement_sections]
        self.logger.info("   Keeping: %s", ', '.join(kept_times))
    ...
    def _rearrange_for_flow(
        ...
        while made_swap and iterations < max_iterations:
            ...
            def key_distance(key1, key2):
                # Simple key distance: 0 if same, 1 if same root but major/minor, 2 if different root
                if key1 == key2:
                    return 0
                if key1 == "unknown" or key2 == "unknown":
                    return 1.5
                root1, mode1 = key1.split()
                root2, mode2 = key2.split()
                if root1 == root2:
                    return 1
                return 2
            ...
            for i in range(len(result) - 1):
                ...
                key_penalty = key_distance(curr['section'].end_key, next_sec['section'].start_key)
                if key_penalty >= 2:
                    continue  # Don't swap if keys are very different
                ...
                if i < climax_pos and curr_energy > next_energy * 1.3:
                    ...
                    if curr_flow_ok:
                        ...
                        made_swap = True
                        self.logger.info("   Swapped sections at positions %d and %d for better energy/key flow", i, i+1)
                        break  # Restart the loop after a swap
        
        # Enhanced dynamic rearrangement logic
        import random

        # Allow non-adjacent swaps based on energy and musical features
        def calculate_swap_score(section1, section2):
            energy_diff = abs(section1['avg_energy'] - section2['avg_energy'])
            key_dist = key_distance(section1['section'].features.get('key', 'unknown'),
                                     section2['section'].features.get('key', 'unknown'))
            return energy_diff + key_dist

        # Shuffle sections slightly to introduce variability
        random.shuffle(result)

        # Attempt swaps to improve flow
        for i in range(len(result) - 1):
            for j in range(i + 1, len(result)):
                swap_score = calculate_swap_score(result[i], result[j])
                if swap_score < 1.5:  # Threshold for acceptable swaps
                    result[i], result[j] = result[j], result[i]
                    made_swap = True

        # Return rearranged sections
        return [(item['section'], item['quality']) for item in result]
    ...
    def _score_section_deep(self, audio_data: np.ndarray, section: Section) -> float:
        ...
        # === 0. ALLIN1/EXTERNAL METRICS (up to +/-18 points) ===
        # Confidence-based bonus (from allin1 or external analyzer)
        confidence = getattr(section, 'confidence', 1.0)
        if confidence is not None:
            conf_bonus = max(0, min(10, 10 * (confidence - 0.7) / 0.3))  # 0.7 = 0 pts, 1.0 = 10 pts
            score += conf_bonus
            if conf_bonus > 7:
                bonuses.append("high_confidence")
            elif conf_bonus < 2:
                penalties.append("low_confidence")

        # Label-based bonus/penalty
        label = getattr(section, 'label', '').lower()
        label_bonus = 0
        if label in ("chorus", "drop"):
            label_bonus += 8
            bonuses.append(label)
        elif label in ("verse", "buildup", "bridge"):
            label_bonus += 5
            bonuses.append(label)
        elif label in ("intro", "outro"):
            label_bonus += 2
        elif label in ("unknown", "section", "other", "noise"):
            label_bonus -= 10
            penalties.append(label)
        # Slight penalty for very short/very long generic sections
        if label == "section" and section.duration < 8.0:
            label_bonus -= 5
        score += label_bonus
        ...
        # Debug output for tuning - show key metrics, confidence, label
        metric_str = " ".join([f"{k}={v:.2f}" for k,v in list(metrics.items())[:6]])
        self.logger.debug("      %0.0fs-%0.0fs: %.1f pts | conf=%.2f label=%s | %s | +%s -%s", section.start_time, section.end_time, score, confidence, label, metric_str, bonuses, penalties)
    ...
    def create_doom_arrangement(
        ...
        self.logger.debug("   Computing phrase features for coherence analysis...")
        ...
        self.logger.debug("   Found %d distinct musical ideas", len(phrase_groups))
        ...
        if style_profile and hasattr(style_profile, 'section_timing'):
            self.logger.info("   Using learned section timing from style")
    ...
    def render_arrangement(
        ...
        if sections_rearranged:
            self.logger.info("   Sections rearranged - using enhanced transition logic")

        # Helper: ensure audio is 2D stereo array (n_samples, 2)
        def _to_stereo(arr: np.ndarray) -> np.ndarray:
            ...
            if a.ndim == 1:
                ...
                # Unexpected ndim, try to flatten
                a = a.reshape(-1)
                a = np.stack([a, a], axis=-1)
            ...
            try:
                ...
                if len(beat_times) > 8:
                    ...
                    if loop_length:
                        ...
                        self.logger.info("   Detected loop/bar length: %.2fs (%.1f beats)", bar_length, bar_length/beat_duration)
                    else:
                        bar_length = beat_duration * 4
                        self.logger.info("   Using 4/4 bar length: %.2fs", bar_length)
                    bar_times = self._generate_bar_times(beat_times, bar_length)
            except Exception as e:
                self.logger.warning("   Beat detection failed: %s", e)
                beat_times = None

        # Ensure blender is initialized (lazy, respects demucs_device)
        if not hasattr(self, 'blender') or self.blender is None or getattr(self.blender, 'demucs_device', None) != getattr(self, 'demucs_device', None):
            self.blender = AITransitionBlender(sample_rate=self.sample_rate, demucs_device=getattr(self, 'demucs_device', None))

        # Prepare a per-run cache for Demucs outputs to avoid repeated separation
        if not hasattr(self, '_demucs_cache_dir') or not os.path.isdir(getattr(self, '_demucs_cache_dir', '')):
            try:
                self._demucs_cache_dir = tempfile.mkdtemp(prefix="demucs_cache_")
                self.logger.info("   Demucs cache directory: %s", self._demucs_cache_dir)
            except Exception:
                self._demucs_cache_dir = None

        # Process each section to find good cut points at BAR boundaries and build output incrementally
        output_audio = None
        prev_stems = None  # Store stems for the previously appended audio region
        ...
        for i, (section, new_start, new_end) in enumerate(arrangement.sections):
            ...
            # Extract audio for the section and normalize shape to stereo
            section_audio = audio_data[start_sample:end_sample]
            section_audio = _to_stereo(section_audio)
            # Separate stems for the current section, using cache when available
            current_stems = None
            cache_path = None
            section_duration = end_time - start_time
            if getattr(self, 'min_demucs_length_s', 0.0) and section_duration < float(self.min_demucs_length_s):
                # Skip Demucs for very short sections
                self.logger.debug("   Skipping Demucs for section %d: %.3fs < %.3fs", i, section_duration, float(self.min_demucs_length_s))
                current_stems = None
            else:
                tmp_wav = None
                try:
                    if getattr(self, '_demucs_cache_dir', None):
                        cache_path = os.path.join(self._demucs_cache_dir, f"section_{start_sample}_{end_sample}.npz")
                        if os.path.exists(cache_path):
                            try:
                                npz = np.load(cache_path, allow_pickle=True)
                                current_stems = {k: npz[k] for k in npz.files}
                            except Exception as e:
                                self.logger.debug("   Failed to load Demucs cache %s: %s", cache_path, e)
                                current_stems = None

                    if current_stems is None:
                        tmp_fd, tmp_wav = tempfile.mkstemp(suffix=".wav")
                        os.close(tmp_fd)
                        sf.write(tmp_wav, section_audio, self.sample_rate)
                        current_stems = self.demucs_separator.separate(audio_path=tmp_wav, device=getattr(self, 'demucs_device', None))

                    # Save to cache if possible
                    if cache_path is not None and current_stems:
                        try:
                            np.savez_compressed(cache_path, **{k: v for k, v in current_stems.items()})
                        except Exception as e:
                            self.logger.debug("   Failed to write Demucs cache %s: %s", cache_path, e)
                except Exception as e:
                    self.logger.warning("   Demucs separation failed for section %d: %s", i, e)
                    current_stems = None
                finally:
                    try:
            ...
            # If this is the first section, initialize output (ensure stereo)
            if output_audio is None:
                output_audio = _to_stereo(section_audio.copy())
                prev_stems = current_stems
                continue

            # Determine blend duration and number of samples
            blend_duration = min(crossfade_duration, (end_time - start_time) / 2)
            n_samples = int(self.sample_rate * blend_duration)
            n_samples = min(n_samples, len(output_audio), len(section_audio))

            # Choose blending method: stem-based if both sides have stems, else raw crossfade
            try:
                if prev_stems and current_stems:
                    # Slice stems to the blend region before passing to blend_sections
                    # prev_stems are from the end of output_audio, current_stems from start of section_audio
                    end_stems_sliced = {}
                    start_stems_sliced = {}
                    for k, v in prev_stems.items():
                        ...
                        # Take the last n_samples from prev stems
                        end_stems_sliced[k] = v[-n_samples:] if v.shape[0] >= n_samples else v
                    for k, v in current_stems.items():
                        if k == '_sr':
                            continue
                        v = np.asarray(v)
                        if v.ndim == 1:
                            v = np.stack([v, v], axis=-1)
                        # Take the first n_samples from current stems
                        start_stems_sliced[k] = v[:n_samples] if v.shape[0] >= n_samples else v
                    # Use AITransitionBlender which expects the end and start slices
                    blended = self.blender.blend_sections(output_audio[-n_samples:], section_audio[:n_samples], blend_duration=blend_duration, end_stems=end_stems_sliced, start_stems=start_stems_sliced)
                else:
                    # Fallback: simple RMS-normalized linear crossfade
                    end_slice = output_audio[-n_samples:].astype(np.float32)
                    start_slice = section_audio[:n_samples].astype(np.float32)
                    fade = np.linspace(0.0, 1.0, n_samples)[:, None]
                    # Match RMS
                    def _rms(x):
                        return np.sqrt(np.mean(x ** 2) + 1e-12)
                    re = _rms(end_slice)
                    rs = _rms(start_slice)
                    if re > 1e-9 and rs > 1e-9:
                        target = 0.5 * (re + rs)
                        end_slice = end_slice * (target / re)
                        start_slice = start_slice * (target / rs)
                    blended = (1 - fade) * end_slice + fade * start_slice

                # Merge: keep everything up to the blend region from output_audio, then blended, then remainder of section
                if n_samples < len(output_audio):
                    prefix = output_audio[:-n_samples]
                else:
                    prefix = np.zeros((0, output_audio.shape[1]), dtype=output_audio.dtype)
                remainder = section_audio[n_samples:]
                output_audio = np.concatenate([prefix, blended, remainder])
            except Exception as e:
                # On any blending failure, fall back to concatenation with a tiny crossfade
                self.logger.warning("   Blending failed for sections around %d: %s", i, e)
                # Tiny linear crossfade
                n = min(int(self.sample_rate * 0.01), len(output_audio), len(section_audio))
                if n > 0:
                    fade = np.linspace(0.0, 1.0, n)[:, None]
                    end_slice = output_audio[-n:].astype(np.float32)
                    start_slice = section_audio[:n].astype(np.float32)
                    blended = (1 - fade) * end_slice + fade * start_slice
                    prefix = output_audio[:-n]
                    remainder = section_audio[n:]
                    output_audio = np.concatenate([prefix, blended, remainder])
                else:
                    output_audio = np.concatenate([output_audio, section_audio])

            # Update previous stems for next iteration
            prev_stems = current_stems
    ...
    def auto_arrange(
        ...
        """
        # Use external sections if provided (e.g., from allin1)
