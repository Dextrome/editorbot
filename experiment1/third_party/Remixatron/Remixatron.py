...
class InfiniteJukebox(object):
    ...
    def __process_audio(self):
        ...
        for b in reversed(beats):
            if len(b.get('jump_candidates', [])) > 0:
        ...
        # jumping is possible.
        if beats[last_chance].get('jump_candidates'):
            beats[last_chance]['next'] = min(beats[last_chance]['jump_candidates'])
        else:
            # No jump candidates anywhere; fall back to a safe next index.
            # Prefer advancing to the next beat if possible, otherwise stay at last_chance.
            if last_chance < len(beats) - 1:
                beats[last_chance]['next'] = last_chance + 1
            else:
                beats[last_chance]['next'] = last_chance
    ...
    def __compute_best_cluster_with_sil(self, evecs, Cnorm):
        ...
        # Determine the maximum sensible cluster count based on available eigenvectors
        # and the number of beats (rows in evecs). Avoid requesting more clusters
        # than we have eigenvector columns or beats - 1 (silhouette requires < n_samples).
        max_possible = min(48, evecs.shape[1], max(3, evecs.shape[0] - 1))

        if max_possible < 3:
            # Not enough data to form multiple clusters. Return a trivial single-cluster
            # labeling so the rest of the pipeline can proceed gracefully.
            labels = np.zeros(evecs.shape[0], dtype=int)
            return (1, labels)

        for n_clusters in range(int(max_possible), 2, -1):
            ...
            # Guard against any unexpected shape issues by ensuring the slice exists
            if Cnorm.shape[1] < n_clusters:
                # Can't compute this cluster size; skip it
                continue
            ...
            try:
                ...
                # If clustering fails for this candidate size, skip it
                continue
            ...
            # the length of the smallest segment in this cluster configuration
            try:
                silhouette_avg = sklearn.metrics.silhouette_score(X, cluster_labels)
            except Exception:
                # If silhouette cannot be computed (e.g., degenerate clusters), skip
                continue

            # compute segment statistics; be defensive if it fails
            try:
                ratio, min_segment_len = self.__segment_stats_from_labels(cluster_labels.tolist())
            except Exception:
                # fallback values
                ratio, min_segment_len = 1.0, 1
