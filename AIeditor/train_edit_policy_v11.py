"""
Edit Policy V11 - Imitation Learning from Human Edits + Reference Style
OPTIMIZED VERSION with parallel processing, caching, and GPU acceleration

Key innovations:
1. ALIGNMENT: Aligns raw input to edited output to extract exact keep/cut labels
2. IMITATION LEARNING: Trains on human editing decisions (behavioral cloning)
3. REFERENCE STYLE: Contrastive learning to match reference track style
4. TRAJECTORY CONTEXT: LSTM encodes editing history for sequential decisions

Performance optimizations:
- Parallel feature extraction with ProcessPoolExecutor
- Feature caching to disk
- Batch matrix operations for alignment
...
CACHE_DIR = Path("./feature_cache/v11")
...
def extract_features_fast(audio: np.ndarray, sr: int = 22050) -> np.ndarray:
    ...
    features.append(float(onset_env.std()))
