...
# =============================================================================
# TRAINER V8 - WITH REFERENCE SONGS (HIGHER WEIGHT)
# =============================================================================

class EditPolicyTrainerV8:
    """
    V8: Reference-enhanced training with higher reference weight.
    
    Uses three sources of data:
    1. Edit files (positive) - what you kept
    2. Reference songs (positive, weight=0.8) - songs you like  
    ...
    def __init__(
        ...
        reference_weight: float = 0.8,  # How much to weight reference samples (increased from 0.5)
    ):
    ...
    def train(
        ...
        logger.info("Training V8 (Reference Enhanced - High Weight)...")
        ...
        # Save
        model_path = self.model_dir / "classifier_v8_best.pt"
        torch.save(self.classifier.state_dict(), model_path)
        logger.info(f"Saved model to {model_path}")
        
        np.save(self.model_dir / "feature_dim_v8.npy", self.base_feature_dim)
...
class V8Editor:
    """V8 editor with reference-enhanced model (higher weight)."""
    ...
    def __init__(self, model_dir: str = "./models"):
        ...
        # Load V8 model
        self.base_feature_dim = int(np.load(self.model_dir / "feature_dim_v8.npy"))
        ...
        self.classifier.load_state_dict(
            torch.load(self.model_dir / "classifier_v8_best.pt", weights_only=True)
        )
        self.classifier.eval()
...
if __name__ == "__main__":
    trainer = EditPolicyTrainerV8(
        input_dir="./training_data/input",
        output_dir="./training_data/desired_output",
        reference_dir="./training_data/reference",
        model_dir="./models",
        reference_weight=0.8  # Reference samples count as 80% weight (higher than V7's 0.5)
    )
