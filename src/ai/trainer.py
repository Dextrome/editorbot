...
class SongTrainer:
    ...
    def add_to_style(self, style_name: str, profile: SongProfile):
        ...
        print(f"   ✓ Added to style '{style_name}' ({len(self.song_profiles[style_name])} songs)")
    ...
    def train_style(
        ...
        print(f"   Using {len(audio_paths)} reference tracks\n")
        ...
        print(f"\n✅ Style '{style_name}' trained successfully!")
        print(f"   Typical structure: {' → '.join(style_profile.typical_section_order)}")
        print(f"   Avg duration: {style_profile.avg_duration/60:.1f} min")
        print(f"   Avg tempo: {style_profile.avg_tempo:.0f} BPM")
    ...
    def save_style(self, style_name: str, filepath: Optional[str] = None):
        ...
        print(f"💾 Saved style '{style_name}' to {filepath}")
    ...
    def load_style(self, style_name: str, filepath: Optional[str] = None):
        ...
        print(f"📂 Loaded style '{style_name}' ({data['style_profile'].num_songs} songs)")
...
class LearnedArranger(SongArranger):
    ...
    def arrange_with_style(
        ...
        if not self.current_style:
            print("⚠ No style loaded, using default arrangement")
